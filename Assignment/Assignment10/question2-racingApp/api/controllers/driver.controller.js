...
module.exports.driverPartialUpdateOne = function (req, res) {
  ...
  Driver.findById(driverId).exec(function (err, driver) {
    ...
    if (err) {
      console.log("Error finding driver: " + driverId);
      response.status = 500;
      response.message = err;
    } else if (!driver) {
      response.status = 404;
    ...
    if (response.status !== 204) {
      ...
      if (req.body.startingYear) {
        driver.startingYear = req.body.startingYear;
      }
...
module.exports.driverDeleteOne = function (req, res) {
  console.log("Delete a driver controller reached.");
  const driverId = req.params.driverId;
